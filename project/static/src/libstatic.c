...
t_buffer* empty_buffer() {
  ...
  memset(nuevoBuffer,
         0,
         sizeof(t_buffer)); // llenamos con ceros para leer mejor en el hex dump
...
t_paquete* paquete_create() {
  ...
  memset(
    nuevo_paquete,
    0,
    sizeof(t_paquete)); // llenamos con ceros para leer mejor en el hex dump
...
t_buffer* crear_mensaje(char* texto) {
  ...
  t_buffer* mensaje = empty_buffer();     // TODO: need free (1)
...
void* paquete_add_mensaje(t_paquete* paquete, t_buffer* nuevo_mensaje) {
  ...
  // tamaño del stream del paquete + tamaño de nuevo_mensaje->stream + tamaño de
  // nuevo_mensaje->size el sizeof(int) es por nuevo_mensaje->size ya que
  ...
  //
  // nos desplazamos en bytes el valor de paquete->buffer->size para no pisar lo
  // último que tenga, luego de lo último agregaremos el nuevo_mensaje->size
