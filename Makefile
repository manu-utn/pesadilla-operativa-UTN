...
##@ Entorno
i install: install-virtualbox install-dev-utils install-lib-cspec install-lib-commons add-user ## Instalar y configurar entorno (unica vez)
ifeq ($(ENTORNO_PROD), false)
	@rsync -rvz . $(DIR_BASE)
...
.PHONY: i install b build r run s stop e exec w watch h help c clean l list
